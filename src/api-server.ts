...
import { accounts, transactions, receipts, profile, photos } from './db/schema';
...
async function readPostgresDB(): Promise<DatabaseSchema> {
  ...
  const currentProfile: CoupleProfile = profs.length > 0 ? {
    user1: profs[0].user1,
    user2: profs[0].user2,
...
// -------------------------------------------------------------
// SEABANK AUTOMATIC DAILY INTEREST SIMULATOR (On-Demand)
// -------------------------------------------------------------
let lastSimulationTime = 0;

async function runSeaBankSimulation() {
  try {
    ...
    console.error('Failed to run SeaBank interest simulator on PostgreSQL:', err);
  }
}
...
app.get('/api/data', async (req, res) => {
  try {
    ...
    // Simulate daily interest if at least 12 seconds have elapsed since last simulation
    const now = Date.now();
    if (now - lastSimulationTime >= 12000) {
      lastSimulationTime = now;
      await runSeaBankSimulation();
    }

    const dbData = await readPostgresDB();
    res.json(dbData);
  } catch (error) {
...
app.delete('/api/accounts/:id', async (req, res) => {
  ...
  try {
    ...
    // Check transactions first (using readPostgresDB to be accurate)
    const dbDataBefore = await readPostgresDB();
